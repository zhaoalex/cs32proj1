...
void City::preachToFlatulansAroundPlayer()
{
    ...
    // game by the last one in the array. If a Flatulan isn't converted, record
    // the instance in history.
    for (int k = 0; k < m_nFlatulans; )
    {
            ...
            coldiff >= -1  &&  coldiff <= 1)
        {
            if (fp->possiblyGetConverted()) { // if conversion succeeds
                ...
                m_nFlatulans--;
            } else { // conversion didn't succeed: record in history
                k++;
                history().record(fp->row(), fp->col());
            }
        }
        else // not orthogonally or diagonally adjacent
        {
            k++;
        }
